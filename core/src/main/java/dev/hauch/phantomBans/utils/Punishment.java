...
/**
 * Represents a punishment (ban, mute, warn, kick, note).
 */
public class Punishment {
    ...
    public Punishment(String name, String uuid, String reason, String operator, PunishmentType type,
                      long start, long end, String calculation, int id) {
        this.name = name;
        this.uuid = uuid;
        this.reason = reason;
    ...
    public static void create(String name, String target, String reason, String operator,
                               PunishmentType type, Long end, String calculation, boolean silent) {
        new Punishment(name, target, reason, operator,
                end == -1 ? type.getPermanent() : type,
    ...
    public void create(boolean silent) {
        if (id != -1) {
            Universal.get().log("!! Failed! PB tried to overwrite the punishment:");
            Universal.get().log("!! Failed at: " + toString());
            return;
        }
        if (uuid == null) {
            Universal.get().log("!! Failed! PB has not saved the " + getType().getName() + " because there is no fetched UUID");
            Universal.get().log("!! Failed at: " + toString());
            return;
        }

        final int cWarnings = getType().getBasic() == PunishmentType.WARNING
                ? (PunishmentManager.get().getCurrentWarns(getUuid()) + 1) : 0;

        DatabaseManager.get().executeStatement(SQLQuery.INSERT_PUNISHMENT_HISTORY,
                getName(), getUuid(), getReason(), getOperator(), getType().name(), getStart(), getEnd(), getCalculation());

        if (getType() != PunishmentType.KICK) {
            try {
                DatabaseManager.get().executeStatement(SQLQuery.INSERT_PUNISHMENT,
                        getName(), getUuid(), getReason(), getOperator(), getType().name(), getStart(), getEnd(), getCalculation());
                try (ResultSet rs = DatabaseManager.get().executeResultStatement(SQLQuery.SELECT_EXACT_PUNISHMENT,
                        getUuid(), getStart(), getType().name())) {
                    if (rs.next()) {
                        id = rs.getInt("id");
                    } else {
        ...
        PhantomBansAPI.firePunishEvent(this, silent);
    ...
    private void announce(int cWarnings) {
        List<String> notification = MessageManager.getLayout(mi.getMessages(),
                ...
                "DATE", getDate(start),
                "COUNT", cWarnings + "");
        mi.notify("pb.notify." + getType().getName(), notification);
    }
    ...
    public void delete(String who, boolean massClear, boolean removeCache) {
        ...
        if (who != null) {
            ...
            mi.notify("pb.undoNotify." + getType().getBasic().getName(), Collections.singletonList(message));
        ...
        PhantomBansAPI.fireRevokeEvent(this, massClear, who);
    }
        ...
        if (getType().isTemp()) {
            ...
            if (diff > 60 * 60 * 24) {
                duration = MessageManager.getMessage("General.TimeLayoutD",
                        getDurationParameter("D", diff / 60 / 60 / 24 + "", "H", diff / 60 / 60 % 24 + "",
                                "M", diff / 60 % 60 + "", "S", diff % 60 + ""));
            } else if (diff > 60 * 60) {
                ...
                duration = MessageManager.getMessage("General.TimeLayoutS",
                        getDurationParameter("S", diff + ""));
            }
        }
        return duration;
    }
    ...
    public String toString() {
        return "Punishment(name=" + getName() + ", uuid=" + getUuid() + ", operator=" + getOperator()
                + ", calculation=" + getCalculation() + ", start=" + getStart() + ", end=" + getEnd()
                + ", type=" + getType() + ", reason=" + getReason() + ", id=" + getId() + ")";
    }
}
